...
public class ContasAReceber extends JFrame {
	...
	ContasAReceber() {
		...
		posicionaObjeto(jrbCliente, 10, 30, 100, 25);
		...
		jrbCliente.setBackground(Color.white);
		jrbData.setBackground(Color.white);
		btgEscolha.add(jrbCliente);
		...
		final ObjectSet<ContaAReceber> contas = bancoDeDados.query(ContaAReceber.class);

		for (ContaAReceber conta : contas) {
			dtmTabela.setRowCount(dtmTabela.getRowCount() + 1);
			dtmTabela.setValueAt(conta.getCliente().getNome(), n, 0);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (jrbCliente.isSelected()) {
					if (jtfPesquisar.getText().isEmpty()) {
						...
						for (ContaAReceber conta1 : contas) {
							if (conta1.getCliente().getNome().equals(jtfPesquisar.getText())) {

								dtmTabela.setRowCount(dtmTabela.getRowCount() + 1);
								dtmTabela.setValueAt(conta1.getCliente().getNome(), linha, 0);
					...
					if (jtfPesquisar.getText().isEmpty()) {
						...
						for (ContaAReceber conta1 : contas) {
							...
							try {
								...
								if (conta1.getDataVencimento().equals(dt1)) {
